...
-- 3. Drop and recreate get_campaign_members_with_email with the id column added
drop function if exists public.get_campaign_members_with_email(uuid);

create function public.get_campaign_members_with_email(cid uuid)
returns table(
  id         uuid,
  user_id    uuid,
  email      text,
